...
class Test_UpdateUserUsecase:
    def test_update_user_usecase(selfs):
        ...
        updated_user = usecase(idUser=1, new_name="Bruno Guirão MPNTM")

        assert updated_user.name == "Bruno Guirão MPNTM"

    def test_update_user_usecase_wrong_idUser(selfs):
        repo = UserRepositoryMock()
        usecase = UpdateUserUsecase(repo=repo)

        with pytest.raises(EntityError):
            usecase(idUser="1", new_name="Bruno Guirão MPNTM")
